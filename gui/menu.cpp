...
int currentPage = 0; // Index of the selected page
...
void imGuiMenu::renderMenu(bool state) {
	...
	ImGui::Begin("", &state, ImGuiWindowFlags_NoDecoration);
	
	// Config
	setStyle();
	// Combo box for selecting pages
	if (ImGui::BeginCombo("Select Page", pages[currentPage])) {
		for (int i = 0; i < IM_ARRAYSIZE(pages); i++) {
			bool isSelected = (currentPage == i);
			if (ImGui::Selectable(pages[i], isSelected))
				currentPage = i;
	...
	if (currentPage == 0) {
		ImGui::BeginChild("Page1", ImVec2(WIDTH - 10, HEIGHT - 10), false);
	...
	if (currentPage == 1) {
		ImGui::BeginChild("Page2", ImVec2(WIDTH - 15, HEIGHT - 10), false);
		ImGui::ColorEdit3("Visable Color", (float*)&espConf.spottedColours);
		ImGui::Dummy(ImVec2(0.0f, textSeparatorSpace));
		ImGui::ColorEdit3("Invisible Color", (float*)&espConf.notSpottedColours);
		ImGui::Dummy(ImVec2(0.0f, textSeparatorSpace));
		ImGui::ColorEdit3("Misc Color", (float*)&espConf.attributeColours);
		ImGui::Dummy(ImVec2(0.0f, textSeparatorSpace));
		ImGui::ColorEdit3("Skeleton Color", (float*)&espConf.skeletonColours);
